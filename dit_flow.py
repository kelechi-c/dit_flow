...
class DiTBlock(nnx.Module):
    def __init__(self, dim: int, attn_heads: int, drop: float = 0.0, rngs=nnx.Rngs(config.seed)):
        super().__init__()
        self.dim = dim
...
class FinalMLP(nnx.Module):
    def __init__(self, hidden_size, patch_size, out_channels, rngs=nnx.Rngs(config.seed)):
        super().__init__()
...
class DiT(nnx.Module):
    def __init__(self, in_channels: int=4, dim: int = 1024, patch_size: tuple = (2, 2), depth: int = 12, attn_heads=16, drop=0.0, rngs=nnx.Rngs(config.seed)):
        super().__init__()
        self.dim = dim
    ...
    def sample(self, label: Array, num_steps: int = 50):
        ...
        for k in tqdm(range(num_steps), desc='Sampling images'):
            x_t = self.flow_step(x_t=x_t, cond=label, t_start=time_steps[k], t_end=time_steps[k + 1])
...
def batch_trainer(epochs, model, optimizer, train_loader):
    ...
    wandb_logger(key="3aef5402e364c9da47508adf8be0664512ed30b2", project_name="dit_jax")
    ...
    for epoch in tqdm(range(epochs)):
        ...
        wandb.log(
            {"loss": train_loss, "log_loss": math.log10(train_loss)}
        )
